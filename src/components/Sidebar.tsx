...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, onLogout, onClose, dragX, isOpen }) => {
    ...
    const allItems = [
        ...
        { id: 'tasks', label: 'Todos', icon: CheckSquare },
    ...
    const renderButton = (item: typeof allItems[number], mobileStyle: boolean = false) => {
        ...
        // Desktop: motion.button with hover/tap animations
        return (
            <motion.button
                key={item.id}
                onClick={handleClick}
                whileHover={{ scale: 1.02, x: 5, backgroundColor: isActive ? 'var(--bg-card)' : 'rgba(127, 127, 127, 0.1)' }}
                whileTap={{ scale: 0.95 }}
                className={mobileStyle ? `bottom-nav-item ${isActive ? 'active' : ''}` : ''}
                style={!mobileStyle ? {
                    display: 'flex',
                    ...
                    fontWeight: isActive ? 600 : 500,
                    color: isActive ? 'var(--accent-primary)' : (theme === 'dark' ? '#cbd5e1' : '#475569'),
                    backgroundColor: isActive ? (theme === 'dark' ? 'rgba(255,255,255,0.05)' : 'var(--bg-card)') : 'transparent',
            ...
            <>
                ...
                >
                    ...
                    <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        ...
                        <button onTouchEnd={(e) => { e.preventDefault(); onLogout(); }} onClick={onLogout} style={{ color: '#ef4444', border: 'none', background: 'none', textAlign: 'left', display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.75rem 1rem', fontSize: '1rem', fontWeight: 500, cursor: 'pointer' }}>
